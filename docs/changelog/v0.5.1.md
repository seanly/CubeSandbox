...
- **WebUI/dashboard build moved into the builder image**: Node.js LTS is now included in `cube-sandbox-builder`, and `make web-build` / `make web-install` / `make web-lint` run inside the builder container. The one-click release script no longer requires a host `npm` install; the web dist is produced inside the builder and copied out via `ONE_CLICK_WEB_DIST_DIR`.
