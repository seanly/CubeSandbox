...
- **构建镜像升级至 Ubuntu 22.04**：统一构建镜像从 `ubuntu:20.04` 升级至 `ubuntu:22.04`，宿主机最低 glibc 要求从 **2.31** 提升至 **2.35**。这将不再支持 Ubuntu 20.04、Debian 11、RHEL / CentOS 8 以及 OpenCloudOS 8 等宿主系统。影响文件包括 `docker/Dockerfile.builder`、`Makefile`、`deploy/one-click/build-release-bundle-builder.sh`、glibc 预检脚本、CI 工作流及相关文档。
- **WebUI/dashboard 构建移入构建镜像**：`cube-sandbox-builder` 镜像已内置 Node.js LTS，`make web-build` / `make web-install` / `make web-lint` 均在构建容器内执行。一键发布脚本不再需要宿主机安装 `npm`；Web dist 在容器内构建后通过 `ONE_CLICK_WEB_DIST_DIR` 导出。
