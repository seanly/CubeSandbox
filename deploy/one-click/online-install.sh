...
check_early_preflight() {
  ...
  if (( gc_major < 2 )) || { (( gc_major == 2 )) && (( gc_minor < 35 )); }; then
    cat >&2 <<EOF
[online-install] ERROR: glibc version ${glibc_ver} is too old (minimum required: 2.35).
[online-install]   The system must have glibc >= 2.35 (Ubuntu 22.04 LTS baseline).
[online-install]   Detected: glibc ${glibc_ver}.
[online-install]   Supported: Ubuntu 22.04+, Debian 12+, RHEL/CentOS 9+, OpenCloudOS 9+.
EOF
    exit 3
  fi
  echo "[online-install] glibc version ${glibc_ver} OK (>= 2.35)" >&2
